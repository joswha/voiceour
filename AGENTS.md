...
- History keeps exactly one transcript open, on a grouped `Section` of its own inside its day, directly under the row that selected it; each day renders as one to three plates and names itself once. Every closed row recedes while a transcript is open (0.45 opacity, 0.8 under Increase Contrast) so the page has one subject. The row is the heading — stamp, outcome mark, word count, two-line preview — and the detail repeats none of it: the actions sit above the text, then the text and the evidence behind it, with Delete across the bar's spacer. There is one Fix/Teach control, titled after the current transcript selection, and ⌘T presses it. Keep the record-level commands — copy, teach, delete — in the row's own context menu: a context menu on the transcript shadows the text view's `Fix / Teach “word”` item, which is the only way to teach the word under the pointer.
