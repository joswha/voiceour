...
- The console window's ground is a *system* material behind unmodified native controls: `NSGlassEffectView(style: .regular)`. It paints no app-palette colour, it clears only `NSWindow.isOpaque` and `backgroundColor`, and Reduce Transparency replaces it with `windowBackgroundColor`. Content keeps its native section plates: text never sits directly on a sampled desktop.
...
- Capture latches runtime errors, active-device disconnects, conversion errors, and WAV write failures. Reject any latched or zero-successfully-written-frame recording before ASR; scan every audio channel for liveness.
...
- Secure output carries the concealed marker, uses host-only pasteboard preparation, remains copy-only, and creates no `recent-sessions.json` row.
- Ordinary paste attempts carry the transient marker. Other copy-only output remains plain string.
- Publish transcript text and privacy markers as one `NSPasteboardItem`. A failed write MUST NOT post Cmd-V, schedule a clear, confirm copying, or announce clipboard delivery.
...
- Durable mute ownership survives launch and later sessions when the recorded device UID cannot resolve. Clear it only after a real restore attempt; refuse a new mute rather than overwrite pending ownership. Disk-loaded and interrupted fades restore partial volumes.
...
- Swift tools version is 6.4; every target uses Swift 6 language mode and the deployment target is macOS 27.
...
| Scene gate | `make ui-snap` |
| Bless intended scene change | `make ui-update` |
| List scenes | `make ui-list` |
| Semantic flow gate | `make ui-flow` |
| Bless intended flow journal | `make ui-flow-update` |
...
- Verification runs inside the flow, not afterwards as a favour. `scripts/verify_bundle.sh` asserts the signed `voiceour-asr` sibling, arm64-only app and helper, packaged resources, `CFBundleIdentifier = com.voiceour.app`, `LSUIElement = true`, a non-empty `NSMicrophoneUsageDescription`, `LSMinimumSystemVersion = 27.0`, and `vtool` `minos 27.x` / `sdk 27.x` on both binaries. The `audio-input` entitlement is present; `app-sandbox` and `input-monitoring` are absent. Never weaken those assertions to make a build pass. Notarization additionally verifies hardened runtime on both the app and helper, the stapled ticket, and `spctl --assess`; release verification includes clean-account permission behavior.
