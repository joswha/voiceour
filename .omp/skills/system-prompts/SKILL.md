...
license: MIT
metadata:
  source: https://github.com/can1357/oh-my-pi/tree/main/.omp/skills/system-prompts
---
...
Voiceour ships no LLM and holds no product prompt, so every target of this skill is agent-facing repository prose:

- `AGENTS.md` — the house contract. Already in this register; treat it as the reference example, not a compression target.
- `.omp/commands/*.md` — slash commands. Written to be executed cold by a subagent that has no conversation: RFC keywords, named verification commands, explicit non-goals.
- `.omp/skills/*/SKILL.md` — the `description` field is retrieval surface and stays natural and keyword-redundant; the body compresses.
- `docs/*.md` — declarative contracts a model reads before editing. Same register, human-readable.

`## Tool Prompt Authoring` above governs prose about the *harness* tools, not Swift API docs. Swift symbol documentation follows `swift-api-design-guidelines-skill` instead. NEVER convert a doc comment in `Sources/` to RFC keywords: it explains a decision to a reader of the code.
