...
Measured upstream in the oh-my-pi coding agent, whose session-title prompt is consumed by LFM2-350M/700M on-device: the runtime prefills `<title>`, stops on `</title>`, caps 20 tokens, and strips quotes, punctuation and stray tags in code. Nothing in this repository consumes a model; the case study is here because the scaffold-don't-instruct lever generalizes to any prompt whose output shape code can enforce.
