...
def make_backend():
    backend_name = os.environ.get("VOICEOUR_ASR_BACKEND", "fake")
...
def start_preload(backend) -> None:
    if os.environ.get("VOICEOUR_PRELOAD") != "1" or not hasattr(backend, "warm_up"):
        return
    ...
    def preload() -> None:
        try:
            ...
            print(f"VOICEOUR_PRELOAD warm in {elapsed_ms}ms", file=sys.stderr)
        except Exception as exc:  # pragma: no cover - best-effort warm path
            print(f"VOICEOUR_PRELOAD failed: {exc}", file=sys.stderr)

    Thread(target=preload, name="voiceour-asr-preload", daemon=True).start()
...
def main() -> int:
    ...
    for line in sys.stdin:
        ...
        if isinstance(parsed, HealthRequest):
            ...
            event = Event()
            thread = Thread(
                target=run_transcribe, args=(parsed, event), name=f"voiceour-asr-{parsed.request_id}", daemon=True
            )
            with inflight_lock:
