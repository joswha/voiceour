...
class ArkBackend(Backend):
    ...
    def __init__(self, spec: cache.ModelSpec, backend_id: str) -> None:
        ...
        # dedicated thread.
        self._ark_thread = ThreadPoolExecutor(max_workers=1, thread_name_prefix="voiceour-ark")
        try:
    ...
    def _warm_up_on_ark_thread(self):
        ...
        mx.clear_cache()
        print(
            f"voiceour-asr ark mem after warm-up: "
    ...
    def _infer_on_ark_thread(self, audio_path: Path, cancelled: Event) -> str | None:
        ...
        try:
            ...
            mx.clear_cache()
            print(
                f"voiceour-asr ark mem: active={active_mb}MB cache={cache_mb}MB peak={peak_mb}MB",
                file=sys.stderr,
            )
