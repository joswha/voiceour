...
from voiceoour_asr.decoding import aligned_result_to_transcript
...
class MLXBackend(Backend):
    ...
    def __init__(self) -> None:
        ...
        self._mlx_thread = ThreadPoolExecutor(max_workers=1, thread_name_prefix="voiceoour-mlx")
        try:
    ...
    def _warm_up_on_mlx_thread(self):
        ...
        mx.clear_cache()
        print(
            f"voiceoour-asr mlx mem after warm-up: "
    ...
    def _infer_on_mlx_thread(
        ...
        try:
            ...
            if use_beam:
                ...
                from voiceoour_asr.decoding.nbest import DEFAULT_BEAM_SIZE, build_nbest_model
            ...
            mx.clear_cache()
            print(
                f"voiceoour-asr mlx mem: active={active_mb}MB cache={cache_mb}MB peak={peak_mb}MB",
                file=sys.stderr,
            )
    ...
    def transcribe(self, request: TranscribeRequest, cancelled: Event):
        ...
        if bias_phrases:
            ...
            from voiceoour_asr.decoding.trie import PhraseTrie
            from voiceoour_asr.decoding.vocabulary import PieceVocabulary
        ...
        try:
            if bias is not None:
                ...
                use_beam = os.environ.get("VOICEOOUR_ASR_DECODING", "greedy").strip().lower() == "beam"
