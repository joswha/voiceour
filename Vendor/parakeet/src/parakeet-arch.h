...
enum parakeet_tensor {
    ...
    // VOICEOUR PATCH: optional per-layer linear biases carried by the bias-variant
    // Conformer checkpoints (parakeet-tdt-1.1b). Absent from the pinned 0.6b files;
    // a file either carries the complete set or none.
...
static const std::map<parakeet_hparam, int32_t> PARAKEET_HPARAM_MODEL_VALUES = {
    ...
    // VOICEOUR PATCH: raised from 24 so the bias-variant 42-layer Conformer
    // checkpoints (parakeet-tdt-1.1b) pass the sanity bound; the pinned model is
    // enforced by the client-side manifest machinery, not this cap.
    {PARAKEET_HPARAM_N_AUDIO_LAYER,            42},
