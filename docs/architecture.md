...
`f16` is the default and the fallback for an unknown or stale persisted value. The two decode the same text to within what the committed corpus can resolve, so the choice is a footprint trade: `q8_0` saves 587 MB of download and approximately 1.17 GB of combined post-load cache and loads faster, `f16` decodes faster. [performance-roadmap.md](performance-roadmap.md) holds the measurements.
