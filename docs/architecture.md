...
A preview is never inserted, and **it is never adopted as the final transcript either**. It was, for one day. The argument was that auto-stop fires only after every sample from `AutoStopDetector.lastLoudAt` onwards has stayed below the 0.08 normalized-RMS threshold for the full dwell, so a preview snapshotted at or after that instant has seen every sample that is not already known to be silence, and re-decoding would spend 400+ ms to produce the same words. That argument is wrong, and measurement is what says so: the encoder is not causal over the utterance, so trailing frames change its representation of earlier ones and the decoder's punctuation, casing and word-joining move with them.

Measured 2026-08-15 on 32 LibriSpeech rows through the shipped sidecar, comparing a decode of the audio up to silence onset against a decode of the same audio plus the 2.5 s dwell: **25 of 32 transcripts differ verbatim** with a digitally silent tail, 21 of 32 with a sub-threshold noise tail, and the normalized divergence scored with the repo's own jiwer + whisper-normalizer is **0.493% U-WER** (quiet) and **0.583%** (noisy). The pinned model sits at 2.81% U-WER on that corpus and the regression gate is 0.35 pp, so adoption spent more than the whole gate budget to save the 347 ms median ASR stage. Observed changes are not cosmetic: `here, and however` → `here. And however`, `new born` → `newborn`, `ou best, When I was certain` → `you best when I was certain`.

`partialAdoptionEnabled` in `PartialTranscription.swift` is therefore `false`. The mechanism stays — the timing rule is still correct and still tested, and `partial-adopted` is still what an adopted session would record — but the switch above it says no, and re-enabling it requires a measurement that clears the gate rather than an argument that sounds right.
