...
| **M3** | `RecordingOverlay.swift` (1,565, 7 subsystems) | `RecordingOverlayController` · `RecordingOverlayLayout` · `RecordingOverlayRendering` · `RecordingOverlayComet` · `RecordingOverlayWaveform`. Also move the pure `RecordingOverlayFramePlacement` (`FocusTracker:150-257`) beside its existing tests | `swift test --filter RecordingOverlayPlacementTests`; `make ui-snap` + `ui-snap-os26` on `overlay.*` scenes |
| **M4** | `GlassKit.swift` (1,721) / `DesignTokens.swift` (678) | GlassKit → window bridge · glass surfaces · marks · control styles. DesignTokens → canonical tokens vs harness instrumentation (`:333-525`). No renames, no access-level changes | `make ui-snap` and `ui-snap-os26` byte-identical; `UISceneCatalogTests` |
| **M5** | `UIHarnessMain.swift` (977) | Scene runner · artifact/golden reconciler · manifest+reporter · `UIHarnessDiff`. CLI parsing already lives correctly in `UIHarnessContracts` | `scripts/ui_harness.sh --except os26 --stdout --no-sheet`; manifest, status counts, and exit codes unchanged |
