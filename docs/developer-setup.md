...
- macOS 27 or newer.
- Swift 6.4 and the macOS 27 SDK, supplied by Xcode 27. Every Swift target uses Swift 6 language mode and the toolchain's `Testing` framework. `swift format` ships inside the toolchain, so `make format` installs nothing.
...
If `make signing` cannot unlock an existing development keychain, it refuses to replace it. Unlock it as its owner; do not delete or reset it for a build. `VOICEOUR_CODESIGN_IDENTITY=- make bundle` provides the documented ad-hoc local fallback, but does not preserve a stable TCC identity or produce a distributable app.

To use another existing signing identity without changing that keychain, set `VOICEOUR_CODESIGN_IDENTITY` and optionally `VOICEOUR_CODESIGN_KEYCHAIN` in the repository's ignored `.env`. Bundling reads these local defaults only when no identity was supplied by the calling environment. An explicit identity bypasses both local signing pins, so release signing does not inherit a development-only keychain. Run `make bundle` after changing the pins. Changing signing identity can require a fresh Microphone or Accessibility grant; subsequent rebuilds should keep using the selected identity.
...
`make check` is the whole local gate in one command. It runs `build`, `format-check`, `check-docs`, `lint-python`, `test`, `ui-flow`, `ui-mercury-bench`, `test-python`, `self-test` and `bench-smoke` in that order.
...
- `make verify-bundle` checks bundle shape, identifiers, resources, arm64 architecture, entitlements, and signatures. `LSMinimumSystemVersion` must be `27.0`; `xcrun vtool -show-build` must report `minos 27.x` and `sdk 27.x` for both the app and helper.
...
Notarization signing enables and verifies the hardened runtime on both the app and `voiceour-asr`. Local development bundles do not enable it.
...
| `make check` | Whole local gate, in order. |
...
| `make ui-snap` | Scene digests and AX dumps. |
| `make ui-update` | Bless intended scene changes. |
| `make ui-list` | List the scene catalogue. |
| `make ui-flow` | Semantic flow journals. |
| `make ui-flow-update` | Bless intended flow journals. |
| `make ui-flow-list` | List the flow catalogue. |
| `make ui-all` | Every scene and semantic flow gate. |
