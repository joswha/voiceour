...
struct SegmentGroup<Content: View>: View {
    ...
    var body: some View {
        SegmentRowsLayout(rows: max(rows, 1), spacing: VoiceOourMetrics.Space.xs) {
            content
        }
        .padding(VoiceOourMetrics.Space.xs)
        .plateSurface(kind: .group, cornerRadius: VoiceOourMetrics.Radius.row)
        .containerShape(.rect(cornerRadius: VoiceOourMetrics.Radius.row))
    }
}
