...
struct ConfirmActionRow: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: VoiceOourMetrics.Space.xs) {
            HStack(alignment: .center, spacing: VoiceOourMetrics.Space.xl) {
                Text(label)
        ...
        .frame(
            maxWidth: .infinity,
            minHeight: didFail ? VoiceOourMetrics.Row.settings : VoiceOourMetrics.Row.table,
            alignment: .leading
        )
    ...
    private var failureLine: some View {
        HStack(alignment: .firstTextBaseline, spacing: VoiceOourMetrics.Space.sm) {
            CaptionText(failureText, color: VoiceOourPalette.Signal.crimson)
    ...
    private func setConfirming(_ isOpen: Bool) {
        if a11y.reduceMotion {
            ...
            withAnimation(VoiceOourMotion.deliberate) { isConfirming = isOpen }
        }
    }
}
