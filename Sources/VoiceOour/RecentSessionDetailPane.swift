...
struct RecentSessionDetailPane: View {
        ...
        ContentCard(eyebrow: "SESSION") {
            VStack(alignment: .leading, spacing: VoiceOourMetrics.Space.lg) {
                if let session = displayedSession {
                    ...
                    if let deleteError {
                        CaptionText(deleteError, color: VoiceOourPalette.Signal.crimson)
                    }
    ...
    private func header(for session: RecentSession) -> some View {
        HStack(alignment: .firstTextBaseline, spacing: VoiceOourMetrics.Space.md) {
            Text(SessionsFormatters.detailStamp.string(from: session.createdAt))
                .font(VoiceOourTypography.title)
    ...
    private var feedbackAnimation: Animation? {
        a11y.reduceMotion ? nil : .easeInOut(duration: VoiceOourMotion.quickDuration)
    }
