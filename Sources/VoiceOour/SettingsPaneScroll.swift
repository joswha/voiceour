...
struct SettingsPaneScroll<Content: View>: View {
    var maxContentWidth: CGFloat = VoiceOourMetrics.Content.form
    let content: Content

    init(maxContentWidth: CGFloat = VoiceOourMetrics.Content.form, @ViewBuilder content: () -> Content) {
...
private struct PaintedBottomScrollEdge: ViewModifier {
    private static let band = VoiceOourMetrics.Space.xl
