...
struct ActivityBarPlot<AxisFooter: View>: View {
    ...
    /// baseline.
    private var minimumBar: CGFloat { VoiceOourMetrics.Space.sm }
    ...
    private var column: UnevenRoundedRectangle {
        UnevenRoundedRectangle(
            topLeadingRadius: VoiceOourMetrics.Radius.keycap,
            bottomLeadingRadius: 0,
            bottomTrailingRadius: 0,
            topTrailingRadius: VoiceOourMetrics.Radius.keycap,
            style: .continuous
        )
    }
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: VoiceOourMetrics.Space.sm) {
            Text(readout)
                .roleStyle(.micro)
                .monospacedDigit()
                .foregroundStyle(activeIndex == nil ? a11y.textLow : VoiceOourPalette.Text.mono)
                .lineLimit(1)
            ...
            ZStack {
                ForEach(bucketValues.indices, id: \.self) { index in
                    ...
                    Text(label)
                        ...
                        .frame(width: VoiceOourMetrics.Space.hair, height: VoiceOourMetrics.Space.hair)
                        .offset(y: VoiceOourMetrics.Space.sm)
                }
            }
        }
        .focusable(true)
        ...
        .overlay {
            if chartFocused {
                RoundedRectangle(cornerRadius: VoiceOourMetrics.Radius.row, style: .continuous)
                    .strokeBorder(
    ...
    private var trackFill: Color {
        a11y.contrast == .increased ? VoiceOourPalette.Plate.hover : VoiceOourPalette.Plate.rest
    }
    ...
    private func setPinned(_ next: Int?) {
        ...
        if a11y.reduceMotion {
            ...
            withAnimation(VoiceOourMotion.quick) { pinnedIndex = next }
        }
    }
    ...
    private func moveKeyboard(by delta: Int) {
        ...
        if a11y.reduceMotion {
            ...
            withAnimation(VoiceOourMotion.quick) { keyboardIndex = next }
        }
    }
