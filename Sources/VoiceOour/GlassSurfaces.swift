...
/// transparent window/panel. VoiceOour keeps the type internal to the app module so
                ...
                ZStack {
                    ...
                    RoundedRectangle(
                        cornerRadius: VoiceOourMetrics.Radius.nested(
                            cornerRadius,
                            inset: VoiceOourMetrics.Stroke.hairline(a11y.contrast)
                        ),
                        style: .continuous
                    )
                    .strokeBorder(
                    ...
                    .padding(VoiceOourMetrics.Stroke.hairline(a11y.contrast))
                }
            }
            .environment(\.surfaceGround, SurfaceGround(a11y.ground))
    }
...
extension View {
    func glassSurface(
        cornerRadius: CGFloat = VoiceOourMetrics.Radius.window
    ) -> some View {
...
struct PlateSurface: ViewModifier {
    ...
    private var fill: Color {
        if kind == .well {
            return VoiceOourPalette.Ink.well
        }
        if kind == .group {
            return Color.clear
        }
        ...
        case .rest:
            return restingFill
        case .hover, .selected:
            return VoiceOourPalette.Plate.hover
        case .pressed:
            return VoiceOourPalette.Plate.pressed
        case .focused:
        ...
        case .disabled:
            return isDisabledSelection ? VoiceOourPalette.Plate.rest : restingFill
        case .inFlight:
            return VoiceOourPalette.Plate.rest
        }
    }
    ...
    private var disabledStroke: Color {
        if isDisabledSelection {
            return VoiceOourPalette.Signal.cyan.opacity(a11y.contrast == .increased ? 0.60 : 0.40)
        }
