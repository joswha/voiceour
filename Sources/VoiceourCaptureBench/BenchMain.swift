...
@main
struct VoiceourCaptureBenchMain {
    static func main() async {
        do {
            ...
            fputs("voiceour-capture-bench: \(error.description)\n", stderr)
            if error.isUsageError {
                fputs("\n\(CaptureBenchCLI.usage)\n", stderr)
            }
            exit(error.exitCode)
        } catch {
            fputs("voiceour-capture-bench: \(error.localizedDescription)\n", stderr)
            exit(1)
        }
    }
}
...
private enum CaptureBenchCLI {
    static let usage = """
        Usage:
          voiceour-capture-bench --mode <standard|native|voice-processing|voice-processing-no-agc|sound-isolation|sound-isolation-high-quality> --duration-ms N --pre-roll-ms N --post-roll-ms N --output PATH
