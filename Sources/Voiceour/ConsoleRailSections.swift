...
struct RailFooter: View {
    ...
    private var statusTint: Color {
        ...
        case .live: VoiceourPalette.Signal.cyan
        case .error: VoiceourPalette.Signal.crimson
        }
    }
    ...
    var body: some View {
        ...
        .padding(.horizontal, VoiceourMetrics.Space.sm)
        .padding(.bottom, VoiceourMetrics.Space.lg)
    }
    ...
    private var identity: some View {
        VStack(alignment: .leading, spacing: VoiceourMetrics.Space.xs) {
            HStack(spacing: VoiceourMetrics.Space.md) {
                statusMark
                    .frame(
                        width: VoiceourMetrics.Icon.navSlot,
                        height: VoiceourMetrics.Icon.navSlot
                    )
            ...
            VStack(alignment: .leading, spacing: VoiceourMetrics.Space.xs) {
                ...
                if coordinator.isSystemAudioMuted {
                    Text("AUDIO MUTED")
                        .roleStyle(.micro)
                        .foregroundStyle(VoiceourPalette.Signal.amber)
                }
            }
            .padding(.leading, VoiceourMetrics.Icon.navSlot + VoiceourMetrics.Space.md)
        }
        ...
        .accessibilityLabel(
            coordinator.isSystemAudioMuted
                ? "Voiceour, \(accessibilityStatus), audio muted"
                : "Voiceour, \(accessibilityStatus)"
        )
    }
    ...
    private var hotkey: some View {
        HStack(spacing: VoiceourMetrics.Space.md) {
            Image(systemName: "mic")
                .font(.system(size: VoiceourMetrics.Icon.row))
