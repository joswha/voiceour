...
struct GlossaryPane: View {
    ...
    var body: some View {
        ...
        SettingsPaneScroll(maxContentWidth: VoiceourMetrics.Content.table) {
            SettingsSectionBlock(eyebrow: "PROJECT LEXICON") {
                ...
                SettingsRow(
                    ...
                    captionColor: importFailure.map { _ in VoiceourPalette.Signal.crimson },
            ...
            SettingsSectionBlock(eyebrow: "TERM LEDGER") {
                LazyVStack(alignment: .leading, spacing: 0) {
                    if terms.isEmpty {
                        ...
                        EmptyState(
                            ...
                            body:
                                "A canonical term is the spelling Voiceour holds a word to when dictation keeps hearing it wrong. Add the first one below, or import a project word list."
                        )
                        .glossaryBand(height: VoiceourMetrics.Row.table * 5, alignment: .leading)
                    } else {
                ...
                .settingsContentLead((VoiceourMetrics.Row.table - 13) / 2)

                if let duplicate {
                    CaptionText("\(duplicate) is already in the ledger.", color: VoiceourPalette.Signal.amber)
                ...
                .padding(.top, VoiceourMetrics.Space.md)
