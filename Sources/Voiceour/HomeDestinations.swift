    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: VoiceourMetrics.Space.sm) {
            Text(readout)
                .roleStyle(.micro)
                .monospacedDigit()
                .foregroundStyle(hoveredIndex == nil ? a11y.textLow : VoiceourPalette.Text.mono)
                .lineLimit(1)
...
struct DestinationRow: View {
    ...
    /// into each other.
    private static let nameWidth = VoiceourMetrics.Space.section * 2
    private static let valueWidth = VoiceourMetrics.Space.section
    ...
    private var nameColor: Color {
        (isHovered || isTop) ? VoiceourPalette.Text.high : a11y.textMid
    }
