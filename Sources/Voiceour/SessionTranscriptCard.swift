...
struct TranscriptCard: View {
    ...
    private static let maximumViewport = VoiceourMetrics.Space.section * 8
    ...
    private var viewportHeight: CGFloat {
        min(max(measuredTextHeight, VoiceourMetrics.Space.lg), Self.maximumViewport)
    }
    ...
    private func metadataRow(_ label: String, _ value: String, mono: Bool = false) -> some View {
        GridRow {
            ...
            Text(value)
                .font(mono ? VoiceourTypography.bodyMono : VoiceourTypography.caption)
                .foregroundStyle(mono ? VoiceourPalette.Text.low : a11y.textMid)
