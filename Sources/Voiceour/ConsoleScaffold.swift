...
struct ConsoleScaffold: View {
    ...
    private var scaffold: some View {
        ...
        .glassSurface(cornerRadius: VoiceourMetrics.Radius.window)
        .overlay(alignment: .top) {
            ...
            .frame(height: VoiceourMetrics.Space.titlebar)
        ...
        .frame(
            minWidth: VoiceourMetrics.Window.minWidth,
            maxWidth: .infinity,
            minHeight: VoiceourMetrics.Window.minHeight,
            maxHeight: .infinity,
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            SectionHeader(section: section, coordinator: coordinator)
                .padding(.horizontal, VoiceourMetrics.Space.xl)
                ...
                .background(alignment: .top) {
                    GroundScrim(taper: VoiceourMetrics.Space.xl)
                        .clipShape(
                            UnevenRoundedRectangle(
                                topTrailingRadius: VoiceourMetrics.Radius.window,
                                style: .continuous
                            )
                        )
                        .padding(.top, -VoiceourMetrics.Space.titlebar)
