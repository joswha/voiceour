...
enum PropertyGrid {
    static let valueOrigin = VoiceourMetrics.Column.settingsLabel + VoiceourMetrics.Space.xl
}
...
struct PropertyRow: View {
    ...
    private func accessoryView(_ accessory: PropertyAccessory) -> some View {
        ...
        case .metadata(let text):
            ...
            Text(text)
                .font(VoiceourTypography.micro.monospacedDigit())
    ...
    private var monoValueForeground: Color {
        isEnabled ? VoiceourPalette.Text.mono : a11y.textLow
    }
    ...
    private var minimumPitch: CGFloat {
        caption == nil ? VoiceourMetrics.Row.table : VoiceourMetrics.Row.settings
    }
