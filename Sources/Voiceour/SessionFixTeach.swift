            ...
            HStack(spacing: VoiceourMetrics.Space.sm) {
                VStack(alignment: .leading, spacing: VoiceourMetrics.Space.hair) {
                    ...
                    TextField(
                        "e.g. kubectl",
                        text: $canonical,
                        prompt: Text("e.g. kubectl").foregroundStyle(VoiceourPalette.Text.low)
                    )
                ...
                VStack(alignment: .leading, spacing: VoiceourMetrics.Space.hair) {
                    ...
                    TextField(
                        "e.g. cube control",
                        text: $misheard,
                        prompt: Text("e.g. cube control").foregroundStyle(VoiceourPalette.Text.low)
                    )
                    .textFieldStyle(GlassTextFieldStyle(font: VoiceourTypography.bodyMono))
...
private struct PendingSuggestionRow: View {
    ...
    var body: some View {
        HStack(alignment: .center, spacing: VoiceourMetrics.Space.md) {
            HStack(spacing: VoiceourMetrics.Space.xs) {
                ...
                Image(systemName: "arrow.right")
                    ...
                    .foregroundStyle(VoiceourPalette.Mark.faint)
            ...
            HStack(spacing: VoiceourMetrics.Space.sm) {
                ...
                Button("ACCEPT", action: accept)
                    .buttonStyle(GlassButtonStyle(kind: .accent))
                    .help(
                        "Teach Voiceour to use \"\(suggestion.canonical)\" next time. Your pasted text is unchanged."
                    )
        ...
        .plateSurface(kind: .row, cornerRadius: VoiceourMetrics.Radius.row)
    }
}
