...
public final class ParakeetSidecarBackend: SidecarBackend, @unchecked Sendable {
    ...
    /// Preview decode of the live `.pcm` tee.
    ///
    /// Deliberately not chunked or streamed: this model is batch-trained, and a measured
    /// chunked decode cost +66 ms and changed 5.56% of words. A preview is therefore a whole
    /// re-decode of the growing buffer, cheap enough below the cap and honest above it.
    public func transcribePartial(
        ...
        let samples: [Float]
        do {
            samples = try PCMFile.readSamples(at: audioURL, limitSampleCount: request.audio.byteCount / 2)
        } catch {
            return .failure(code: .unsupportedAudioFormat, detail: String(describing: error), fatal: false)
        }

        // Below 1.5 s there is not enough audio for a preview worth showing; above 120 s the
        // preview's value has plateaued and its cost has not. Both answer an empty transcript
        // rather than an error: nothing is wrong, there is simply nothing to preview.
        guard samples.count >= Self.minimumPartialSamples, samples.count <= Self.maximumPartialSamples else {
            return emptyPartialResult()
        }
        ...
        do {
            ...
            return .failure(code: .modelLoadFailed, detail: String(describing: error), fatal: false)
        }
        ...
        do {
            ...
            if isCancelled() { return .cancelled }
            // Never fatal, whatever went wrong: a preview must not be able to kill the process
            // the user's real dictation is about to need. The final decode decides fatality.
            return .failure(code: .inferenceFailed, detail: String(describing: error), fatal: false)
        }
        let inferenceMs = Int((DispatchTime.now().uptimeNanoseconds - started) / 1_000_000)
    ...
    /// 1.5 s at 16 kHz.
    static let minimumPartialSamples = 24_000
    /// 120 s at 16 kHz.
    static let maximumPartialSamples = 1_920_000

    private func emptyPartialResult() -> SidecarTerminal {
        stateLock.lock()
        let loadMs = self.loadMs
        stateLock.unlock()
        return .result(
