...
APP=$(VOICEOOUR_CODESIGN_IDENTITY="$IDENTITY" "$ROOT/scripts/bundle.sh")
codesign --force --options runtime --entitlements "$ROOT/Resources/VoiceOour.entitlements" --sign "$IDENTITY" "$APP"
codesign --verify --deep --strict --verbose=2 "$APP"
codesign --display --entitlements :- "$APP"
"$ROOT/scripts/verify_bundle.sh"
ZIP="$ROOT/.build/VoiceOour.zip"
ditto -c -k --keepParent "$APP" "$ZIP"
if [ -n "$NOTARY_KEYCHAIN_PROFILE" ]; then
  xcrun notarytool submit "$ZIP" --keychain-profile "$NOTARY_KEYCHAIN_PROFILE" --wait
else
  echo 'Warning: APPLE_APP_SPECIFIC_PASSWORD is visible in the process table during notarization; run once: xcrun notarytool store-credentials "VoiceOour-notary", then set NOTARY_KEYCHAIN_PROFILE=VoiceOour-notary.' >&2
...
manifest="$ROOT/.build/VoiceOour-release-manifest.txt"
cat > "$manifest" <<EOF
app_path=$APP
zip_path=$ZIP
