...
The glossary protects exact technical terms through cleanup. Add a canonical spelling with one or more ways the recognizer may hear it, import a project word list, or teach a correction straight from History: select any part of a transcript and the teach button names what it will teach, or right-click a word for `Fix / Teach “word”`. Cmd-T opens the same editor. History detail also names the word the recognizer was least sure of, with a Teach… shortcut, so a wrong word can be corrected with the evidence attached. Teaching writes the glossary and changes future dictation only; text you already pasted is untouched.
