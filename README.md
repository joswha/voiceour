...
The window uses a native `TabView` on a glass ground: the material is the system's (Liquid Glass on macOS 26, behind-window vibrancy below it), the controls are stock macOS, and Reduce Transparency swaps the material for a plain window background. Home reports what this Mac has dictated — total time, words, time saved against a 40 wpm typing baseline, average speaking speed, the five apps that receive the most of your dictation under their own icons, active days, streaks, and an activity grid whose squares name their own day and word count the moment you point at one — on its own dark glass islands. The other four are grouped Forms: General owns capture/cleanup/audio settings; Glossary owns terms and imports; History owns search, an app filter and day-grouped transcripts that name where each one went with that app's icon beside its name, and opens one of them on its own plate directly under the row you click — the full text with no buttons around it, with every other row dimmed while it is open; System owns readiness, permissions, diagnostics, and destructive clears. Every app is called what you call it: the name macOS shows for the installed app, falling back to the name recorded with the dictation when the app is gone.
...
- **Stats:** `dictation-activity.json` stores the counts Home reports — per local day, per destination app, and in total — and nothing that identifies an utterance: no transcript text and no session ids. The per-app entries are counts under a bundle id and its display name, which is what lets Home rank destinations past the 500-transcript cap. A dictation into a secure field is not counted at all. Clear History removes this too.
...
The glossary protects exact technical terms through cleanup. It reads as a searchable list of terms: search over terms and their spoken forms, narrow to the ones you added or the ones Voiceour ships, and click a term to open it in place — `Term` is the spelling dictation is held to, `Heard as` lists the spoken forms it maps, and `Also matched` shows the case and spacing variants matched without typing them. `Add Term` composes a new one and Remove Term confirms first. You can also import a project word list, or teach a correction straight from History: the open transcript carries no action buttons, so select any part of it and press ⌘T — the line under the text names what it will teach — or right-click a word for `Fix / Teach “word”`. Teaching opens the same `Term` / `Heard as` editor. A plain single click on the transcript copies the whole thing and confirms in place. History also names the word the recognizer was least sure of, with its probability, so a wrong word can be corrected with the evidence attached, and keeps the pre-cleanup text folded away under `Raw` for the rare read where what cleanup changed matters; copying, teaching and deleting a whole record stay in the row's own context menu. Teaching writes the glossary and changes future dictation only; text you already pasted is untouched.
